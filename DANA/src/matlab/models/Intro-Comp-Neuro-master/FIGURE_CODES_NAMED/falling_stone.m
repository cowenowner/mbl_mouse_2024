...
v_sim = zeros(size(t));         % vector to store simulated velocity
y_sim = zeros(size(t));         % vector to store simulated position
for i = 2:length(t)             % loop through all time-points
    v_sim(i) = v_sim(i-1) + dt*a;   % velocity changes by acceleration x time-step
    y_sim(i) = y_sim(i-1) + dt*0.5*(v_sim(i-1)+v_sim(i));  % position changes by mean velocity x time-step
end
...
% In figure 1 plot analytic results as in the table, this is Figure 1.6
figure(1)
clf
subplot(2,1,1)
plot(t,v,'k')
...
% In figure 2 plot the simulated result together with the analytic result
figure(2)
subplot(2,1,1)
plot(t,v_sim)
hold on
