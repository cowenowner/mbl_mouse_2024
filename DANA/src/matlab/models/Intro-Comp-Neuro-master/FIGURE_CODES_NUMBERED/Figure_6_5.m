...
% Synaptic feedback also saturates.
% Plots firing rate and feedback curves to show fixed points then simulates
% the model with inputs.
% Three cases with different degrees of synaptic saturation are depicted.
%
% This code was used to produce Figure 6.5 in the textbook:
...
x_mid = 0.4;                % Default input for 1/2-max response (can vary)
...
%  First trial has each spike generating 1/5 the max. postsynaptic response.
%  Second trial has each spike generating 1/2 the max. postsynaptic response.
%  Third trial has the maximal postsynaptic response following each spike.


%% Synaptic response parameters

alpha_vec = [0.2 0.5 1];        % Baseline postsynaptic amplitude.
Wee_vec = [1.2 0.75 0.55];      % Strength of synaptic feedback.
pr = 1;                         % Vesicle release probability.
tau_s = 0.050;                  % Synaptic time constant for NMDA receptors
...
stim_amp = 0.5;             % Amplitude of stimulus
...
for trial = 1:Ntrials;         
    alpha = alpha_vec(trial);  % Value of baseline postsynaptic amplitude
    ...
    for i = 2:Nt;                               % Loop through time points
        ...
        s(trial,i) = s(trial,i-1)*(1-dt/tau_s) + ...
            dt*pr*alpha*r(trial,i-1)*(1-s(trial,i-1));
