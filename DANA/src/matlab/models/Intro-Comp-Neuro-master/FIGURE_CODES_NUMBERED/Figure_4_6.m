% Figure_4_6.m
% This code loops through several sizes of current step to demonstrate the
% type-II behavior of the Hodgkin-Huxley model.
% This model is the Hodgkin-Huxley model with initial parameters in modern
% SI units (flipping the sign of V and shifting by -0.070V)
% but with V converted to old units (and mV) for calculation of
% the rate constants using the original Hodgkin-Huxley equations.
%
% This code was used to produce Figure 4.5 in the textbook
...
ilength=0.4;        % length of applied current pulse (sec)
Ibase = 0;          % Baseline current before pulse

%% Neuron parameters
V_L = -0.059387;        % leak reversal potential (V)
...
Ievec = [0.2 0.3 0.6 0.624 0.626]*1e-9;      % series of applied currents (A)
