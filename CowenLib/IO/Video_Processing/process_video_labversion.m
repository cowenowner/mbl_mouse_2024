function process_video_labversion(vid_dir, epoch_times, check_flag, headstage_rad,inclusion_rad);
% process_video_labversion  generate pvd (position-velocity-direction) file from video file
...
%      extractvidpos.m - extracts video position via fitting points to a circle
...
if nargin < 5
    inclusion_rad = 40;   % pixel units
end

[path name ext] = fileparts(vid_dir);
[codepath name ext] = fileparts(which(mfilename));
...
if need_VT & ~exist(vid_in, 'file')
   if exist(vid_zip_in, 'file')
		...
		eval(['!' codepath '\unzip ' vid_dir '\VT1.zip -d ' vid_dir]);
...
if exist(ascii_out1, 'file')
   ...
   if isempty(epoch_times)
       cmd = ['!' codepath '\cutvideo -inp ' vid_in ' -outp ' ascii_out1 ' -start 1 -end 99999999999999' ];
   else
       cmd = ['!' codepath '\cutvideo -inp ' vid_in ' -outp ' ascii_out1 ' -start ' num2str(maze1_starttime,11) ' -end ' num2str(maze1_endtime, 11)];
   end
...
if maze2_flag  
   if exist(ascii_out2, 'file')
      disp(['ASCII Video file exists, using: ' ascii_out2]);
   else
		disp('Translating VT file to ASCII');
        maze2ascii_flag = 1;  % this lets us know to erase the ascii file at the end of processing
        if ismepty(epoch_times)
            cmd = ['!' codepath '\cutvideo -inp ' vid_in ' -outp ' ascii_out2 ' -start 1 -end 99999999999999' ];
        else
            cmd = ['!' codepath '\cutvideo -inp ' vid_in ' -outp ' ascii_out2 ' -start ' num2str(maze2_starttime,11) ' -end ' num2str(maze2_endtime, 11)];
        end
...
extractvidpos(infile,  outfile, headstage_rad, inclusion_rad, check_flag, 1);

if maze2_flag
	infile = ascii_out2
	outfile = [vid_dir '\vtm2.pvd']
	extractvidpos(infile,  outfile, headstage_rad, inclusion_rad, check_flag, 1);
end
