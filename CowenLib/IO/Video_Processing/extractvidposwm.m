function pos_data = extractvidpos(infile, outfile, error_count_threash, inclusion_rad, check_flag, smooth_flag)

% EXTRACTVIDPOSWM takes an ascii file of tracker data and returns the rat's position
%                 uses weighted mean (instead of circle fit)
% y = extractvidpos(infile, outfile, error_count_threash, inclusion_rad, check_flag, smooth_flag)
% error_count_threash - program keeps count of how many times in a row the rat is lost
%                       meaning that pixels outside inclusion radius outnumber pixels inside 
%                       if rat is lost for more times than this, relocalization is attempted including all points.
%                       setting this number to zero means the area with most pixels on will be tracked at all times
...
%                 random locations.  Weighted mean fitting operates only on the pixels within
%                 this radius.
...
%                 smoothing done after initial extraction
% 
...
% locally weighted average (using all points in the frame, not just those in teh inclusion radius)
% to find the current position of the rat and then starts tracking from this new position.
%
% INFILE DETAILS:
...
dyc = 0;
track_error = 0;  % this tracks how many times excluded pixels outnumber included pixels
...
while ~end_of_file
   ...
   if first_frame_flag  
      ...
      % start by looking over the first five frames
      ...
      lines_red = lines(lines(:,3)==1,:);  % note we're choosing raw red, not pure red
      lines_green = lines(lines(:,4)==1,:);  % this is raw red
      all_lines = [lines_lum; lines_red; lines_green];
      %all_lines = lines_red; 
      
      [wcx, wcy, raster_data, smooth_raster_data] = find_lwc(all_lines(:,9:10), frame_height, frame_width, 8);
   ...
   % MAIN EXTRACTION LOOP
	for j = 1:last_ts_i
      ...
      cur_ts = tslist(j);
		...
		red_lines = cur_frame_data(logical(cur_frame_data(:,3)),:);  % note we're choosing raw red, not pure red
					
		cur_pixels = [lum_lines(:,9:10); green_lines(:,9:10); red_lines(:,9:10)];
      %    cur_pixels = [red_lines(:,9:10)];
      ...
      if size(cur_pixels,1)==0
		...
		if j~=last_ts_i
			...
			red_lines = next_frame_data(logical(next_frame_data(:,6)),:);  % note we're choosing pure red, not raw red
					
			%next_pixels = [lum_lines(:,9:10); green_lines(:,9:10); red_lines(:,9:10)];
         next_pixels = [red_lines(:,9:10)];
      ...
      % Check to see if we've excluded more points than we've included.  If so start gathering evidence for a shift
      if num_excl>num_incl  % ok we might have a problem, better search again for the rat
         track_error = track_error + 1;
         disp(['WARNING: ' num2str(cur_ts, 11) ' MORE POINTS EXCLUDED THAN INCLUDED.  ERROR COUNT = ' num2str(track_error)])
      else
         track_error = max(track_error - .5, 0);
      end   
      
      % Backup Plan: Find Rat Via Local Weighted Mean on All Points
      if track_error>=error_count_threash
         track_error = error_count_threash/2;  % reset track error, since we presumably are finding the rat again
                                               % note that often when we've lost rat, we choose the wrong position
                                               % (e.g., if the rat tilts his headstage off camera field, just the boom 
                                               % will be visible) so we set the track_error to half the threashold value
                                               % if this is the right value, the continued subtraction from track_error
                                               % for every correct tracking will reduce this number.  If this is the wrong
                                               % position, we'll quickly build up evidence of the error and bounce the 
                                               % extracted position away from the erronious location
         ...
         [wcx, wcy, raster_data, smooth_raster_data] = find_lwc(cur_pixels, frame_height, frame_width, 8);
      ...
      % round to nearest integer because find_lwc requires indices of pixels, which must be integer
      fit_pixels = round(fit_pixels);
		
		%[xc yc dpjump] = fit_circle(fit_pixels(:,1), fit_pixels(:,2), last_xc+dxc, last_yc+dyc, headstage_rad);
		%if isinf(xc)
      %   break;
      %end;
      [xc, yc, raster_data, smooth_raster_data] = find_lwc(fit_pixels, frame_height, frame_width, 8);
      ...
      pos_data_counter = pos_data_counter+1;
		...
		if check_flag>=1
         ...
         hold on;
         plot(xc, yc, '+g');
         draw_circle([xc yc], inclusion_rad, 360, 'g');
         hold off;
         ...
         axis([0 frame_width 0 frame_height]);
		   hold off;
		   text(5,15,num2str(cur_ts), 'color', 'w');
