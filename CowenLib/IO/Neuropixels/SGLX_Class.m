% =============================================================
% COWEN: modified so that this is now a useable class
% obj = SGLX_Class;
% obj.DemoReadSGLXData()
...
classdef SGLX_Class
    methods(Static)
        function DemoReadSGLXData()
            ...
            nSamp = floor(1.0 * SGLX_Class.SampRate(meta));
            dataArray = SGLX_Class.ReadBin(0, nSamp, meta, binName, path);
            ...
            if dataType == 'A'
                if strcmp(meta.typeThis, 'imec')
                    dataArray = SGLX_Class.GainCorrectIM(dataArray, [ch], meta);
                else
                    dataArray = SGLX_Class.GainCorrectNI(dataArray, [ch], meta);
                end
                plot(dataArray(ch,:));
            else
                digArray = SGLX_Class.ExtractDigital(dataArray, meta, dw, dLineList);
        ...
        function [dataArray,meta] = ReadBinVolts(samp0, nSamp, meta, binName, path)
            ...
            if strcmp(meta.typeThis, 'imec')
                ...
                dataArray = SGLX_Class.GainCorrectNI(dataArray, 1:nChan, meta);
            end
            dataArray = single(dataArray);
        end % ReadBinVolts


        function dataArray = WriteBin(samp0, nSamp, meta, binName, path, data)
            ...
            [fid,err] = fopen(fullfile(path, binName),'r+');
            fseek(fid, samp0 * 2 * nChan, 'bof');
            dataArray = fwrite(fid, int16(data), 'int16');
            fclose(fid)
        end % WriteBin
        ...
        function digArray = ExtractDigital(dataArray, meta, dwReq, dLineList)
            ...
            if strcmp(meta.typeThis, 'imec')
                ...
                [MN,MA,XA,DW] = SGLX_Class.ChannelCountsNI(meta);
                if dwReq > DW
        ...
        function [APgain,LFgain] = ChanGainsIM(meta)
            ...
            if (probeType == 21) || (probeType == 24)
                [AP,LF,~] = SGLX_Class.ChannelCountsIM(meta);
        ...
        function dataArray = GainCorrectNI(dataArray, chanList, meta)
            ...
            [MN,MA] = SGLX_Class.ChannelCountsNI(meta);
            fI2V = SGLX_Class.Int2Volts(meta);

            for i = 1:length(chanList)
                j = chanList(i);    % index into timepoint
                conv = fI2V / SGLX_Class.ChanGainNI(j, MN, MA, meta);
        ...
        function dataArray = GainCorrectIM(dataArray, chanList, meta)
            ...
            chans = SGLX_Class.OriginalChans(meta);
            [APgain,LFgain] = SGLX_Class.ChanGainsIM(meta);
            nAP = length(APgain);
            nNu = nAP * 2;
