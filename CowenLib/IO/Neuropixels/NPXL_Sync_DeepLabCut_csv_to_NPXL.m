function [POS, POS_info] = NPXL_Sync_DeepLabCut_csv_to_NPXL(dlc_csv_file_path, NPXL_sync_times, camera_name, varargin)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% function OUT = NPXL_Sync_DeepLabCut_csv_to_NPXL(dlc_csv_file_path,
%                 NPXL_sync_times, camera_name, varargin)
%
...
%   list of sync timestamps (IN uS) recorded on the NPXL system. There should be a
...
if PLOT_IT
    bad_ix = find(diff(NPXL_sync_times)>thresh_uS);
    
    figure
    subplot(2,3,1:3)
    plot(NPXL_sync_times/60e6,zeros(size(NPXL_sync_times)),'.','MarkerSize',1)
    hold on
    plot(NPXL_sync_times(bad_ix)/60e6,zeros(size(bad_ix)),'ro')
    
    xlabel('minutes')
    subplot(2,3,4)
    plot(TBL.Nose_x,TBL.Nose_y,'.','MarkerSize',1)
    subplot(2,3,5)
    histogram(diff(NPXL_sync_times)/1000)
    xlabel('NPXL inter-pulse ms')
    ...
    if ~all_OK
        ...
        figure
        plot(NPXL_sync_times,zeros(size(NPXL_sync_times)),'.','MarkerSize',5)
        hold on
        plot(NPXL_sync_times(bad_ix),zeros(size(bad_ix)),'ro')
        
        title('Click somewhere WITHIN each good block. Will backtrack to start.')
        x = ginput(answer);
        x = x(:,1);
        %%%%%%%%%%
        diffs = [thresh_uS*.9 diff(NPXL_sync_times(:))'];
        first_last_ix = nan(length(x),2);
        good_ix = [];

        for ii = 1:length(x)
            ix_first = find(diffs(:) > thresh_uS & NPXL_sync_times(:)<x(ii),1,'last');
            if isempty(ix_first)
                ix_first = 1;
            end
            ix_last = find(diffs(:) > thresh_uS & NPXL_sync_times(:)>x(ii),1,'first');
            if isempty(ix_last)
                ix_last = length(NPXL_sync_times);
            else
        ...
        disp(first_last_ix)
        updated_frame_times_uS = NPXL_sync_times(good_ix);
...
if ~isempty(updated_frame_times_uS)
    NPXL_sync_times = updated_frame_times_uS;
end

IPIms = diff(NPXL_sync_times)/1000;
sprintf('n vid recs: %d, n ttl pulses %d, min IPI %f, max IPI %f',size(TBL,1),length(NPXL_sync_times), ...
    min(IPIms),max(IPIms))
d = length(TBL.Nose_x) - length(NPXL_sync_times);
...
min_ix = min([Rows(TBL) length(NPXL_sync_times)]);
Intan_uS = nan(size(TBL,1),1);
Intan_uS(1:min_ix) = NPXL_sync_times(1:min_ix);
