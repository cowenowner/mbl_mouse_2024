function NPXL_NP2_Denoise_ap_bin_file(varargin)
...
[channelInds, ~] = imec.lookup_channelIds(imec.channelIds);
...
%For NP2 sort channels by location and shank
channelInds_sorted=channelInds;
shanks = cell(4,1);
uniqueX = unique(imec.channelMap.xcoords);%Unique x coords based on shank
[sortedX, ~] = sort(uniqueX);
for i = 1:length(shanks)
    currentXIndices = find(imec.channelMap.xcoords == sortedX(2*i-1) | imec.channelMap.xcoords == sortedX(2*i));
    % Sort based on y coords(height on shank)
    [~, sortOrder] = sort(imec.channelMap.ycoords(currentXIndices));
    % Store indices in the shanks cell array according to sorted ycoords
    shanks{i} = channelInds_sorted(currentXIndices(sortOrder));
end
...
for shankNum = 1:length(shanks)
    fprintf('Processing Shank %d\n', shankNum);
    shankChannels = shanks{shankNum};
    train_ix = round(linspace(1, double(imec.nSamplesAP), n_training_data)); % 1000 points is good enough for the model
    train_data = single(mmap.Data.x(shankChannels, train_ix)); % access a specific sample
    test_ix = round(linspace(30000, double(imec.nSamplesAP), n_training_data));% 1000 points is good enough for the model
    test_data = single(mmap.Data.x(shankChannels, test_ix));% access a specific sample
    
    %Apply the bad channel mask inside 
    % Create masks for bad channels and apply them to the current shank
    badchanMask = ismember(shankChannels, imec.badChannels);
    if any(badchanMask)
        fprintf( 'Zeroing out bad channels in %f hrs of total data. \n',hrs)
        mmap.Data.x(badchanMask,:) = 0;
    end
    chanMask = ~badchanMask;  % Channels that are not marked as bad
    good_ch_ix = find(chanMask);  % Indices of good channels within the shank
    good_ch_ix(good_ch_ix == 385) = []; % make ch 385 a 'bad channel' as this contains the sync pulse.

    disp('Starting model creation.')
    mod = cell(length(shankChannels), 1);  % Models for each channel
    all_ch_mask = cell(length(shankChannels), 1);  % Channel masks
    allB = cell(length(shankChannels), 1);  % Regression coefficients
    y_hat = zeros(size(train_data, 1), length(good_ch_ix), 'single');  % Placeholder for predictions

    
     for iCh = 1:length(good_ch_ix)
        row_ix = good_ch_ix(iCh);

        permitted_ch_mask = shankChannels < (row_ix-forbidden_radius) | shankChannels > (row_ix+forbidden_radius);
        ...
        bad_outer = shankChannels > (row_ix+outer_radius) | shankChannels < (row_ix-outer_radius);
