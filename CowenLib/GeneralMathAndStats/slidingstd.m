function out = slidingstd(in, N)
%   OUTPUT_ARRAY = SLIDINGstd(INPUT_ARRAY, N)
%NOTE: THIS IS VERY SLOW -- USE SLIDINGSTD_FASTEST IF IT'S TIME CRITICAL.
% noted by COWEN 2014.
%  The function 'slidingstd' implements a one-dimensional filtering, applying a sliding window to a sequence. Such filtering replaces the center value in
...
%  of N larger or equal to 2*(nx - 1), each value of the output data array are identical and equal to nanstd(in).
%
...
%
% COWEN 2014 - added nanstd to this.
% Two simple examples with second- and third-order filters are
%  slidingnanstd([4 3 5 2 8 9 1],2)   
%  ans =
%   3.5000  4.0000  3.3333  5.0000  6.3333  6.0000  5.0000
%
%  slidingstd([4 3 5 2 8 9 1],3)
%  ans =
%   3.5000  4.0000  3.3333  5.0000  6.3333  6.0000  5.0000
%

if (isempty(in)) || (N<=0)                                              % If the input array is empty or N is non-positive,
 sprintf('Slidingstd: (Error) empty input data or N null.');     % an error is reported to the standard output and the
...
nx   = length(in);             % The length of the input data structure is acquired to later evaluate the 'nanstd' over the appropriate boundaries.

if (N>=(2*(nx-1)))                                                     % If the number of neighbouring points over which the sliding 
 out = nanstd(in)*ones(size(in));                                        % average will be performed is large enough, then the average actually covers all the points
...
for i=1:nx,                                                 % For each element (i-th) contained in the input numerical array, a check must be performed:
 if ((i-m) < 1) && ((i+m) <= nx)                             % If not enough points are available on the left of the i-th element..
  out(i) = nanstd(in(1:i+m));                                 % then we proceed to evaluate the nanstd from the first element to the (i + m)-th.
 elseif ((i-m) >= 1) && ((i+m) <= nx)                        % If enough points are available on the left and on the right of the i-th element..
  out(i) = nanstd(in(i-m:i+m));                               % then we proceed to evaluate the nanstd on 2*m elements centered on the i-th position.
elseif ((i-m) >= 1) && ((i+m) > nx)                          % If not enough points are available on the rigth of the i-th element..
  out(i) = nanstd(in(i-m:nx));                                % then we proceed to evaluate the nanstd from the element (i - m)-th to the last one.
 elseif ((i-m) < 1) && ((i+m) > nx)                          % If not enough points are available on the left and on the rigth of the i-th element..
  out(i) = nanstd(in(1:nx));                                  % then we proceed to evaluate the nanstd from the first element to the last.
 end % if
end % for i
