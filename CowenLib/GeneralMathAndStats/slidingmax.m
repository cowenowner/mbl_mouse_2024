function out = slidingmax(in, N)
%   OUTPUT_ARRAY = SLIDINGmax(INPUT_ARRAY, N)
%
%  The function 'slidingmax' implements a one-dimensional filtering, applying a sliding window to a sequence. Such filtering replaces the center value in
...
%  of N larger or equal to 2*(nx - 1), each value of the output data array are identical and equal to max(in).
%
...
%
% COWEN 2014 - added max to this.
% Two simple examples with second- and third-order filters are
%  slidingmax([4 3 5 2 8 9 1],2)   
%  ans =
%   3.5000  4.0000  3.3333  5.0000  6.3333  6.0000  5.0000
%
%  slidingmax([4 3 5 2 8 9 1],3)
%  ans =
%   3.5000  4.0000  3.3333  5.0000  6.3333  6.0000  5.0000
%

if (isempty(in)) || (N<=0)                                              % If the input array is empty or N is non-positive,
 sprintf('Slidingmax: (Error) empty input data or N null.');     % an error is reported to the standard output and the
...
nx   = length(in);             % The length of the input data structure is acquired to later evaluate the 'max' over the appropriate boundaries.

if (N>=(2*(nx-1)))                                                     % If the number of neighbouring points over which the sliding 
 out = max(in)*ones(size(in));                                        % average will be performed is large enough, then the average actually covers all the points
...
for i=1:nx,                                                 % For each element (i-th) contained in the input numerical array, a check must be performed:
 if ((i-m) < 1) && ((i+m) <= nx)                             % If not enough points are available on the left of the i-th element..
  out(i) = max(in(1:i+m));                                 % then we proceed to evaluate the max from the first element to the (i + m)-th.
 elseif ((i-m) >= 1) && ((i+m) <= nx)                        % If enough points are available on the left and on the right of the i-th element..
  out(i) = max(in(i-m:i+m));                               % then we proceed to evaluate the max on 2*m elements centered on the i-th position.
elseif ((i-m) >= 1) && ((i+m) > nx)                          % If not enough points are available on the rigth of the i-th element..
  out(i) = max(in(i-m:nx));                                % then we proceed to evaluate the max from the element (i - m)-th to the last one.
 elseif ((i-m) < 1) && ((i+m) > nx)                          % If not enough points are available on the left and on the rigth of the i-th element..
  out(i) = max(in(1:nx));                                  % then we proceed to evaluate the max from the first element to the last.
 end % if
end % for i
