...
void get_wv_params(float *y, float *p){
	float x[32];
    float y2[32];
    float x_sp[UPSAMPLE_POINTS];
    float y_sp[UPSAMPLE_POINTS];
	...
	for(int m = 1; m<UPSAMPLE_POINTS; m++)
	{
		xx = ((float) m)/UPSAMPLE_POINTS * 5 + start_t; // assme the peak is between 6 and point 11
	...
	for(int m = 1; m<UPSAMPLE_POINTS; m++)
	{
		xx = ((float) m)/UPSAMPLE_POINTS * 15.0 + start_t; // assme the trough is at least 2 points in front of the peak.
		splint(x-1, y-1, y2-1, 32, xx, &y_sp[m]);
		if ((y_sp[m] - y_sp[m-1]) > 0 )
		{
			trough = y_sp[m-1];
