function timestamps = Artificial_spikes(freq,duration,refractory_per,model,out_units)
...
freq
nSpikes = round(freq * duration);
switch out_units
case {'1msec' , 'msec'}
  total_time = duration*1000;  % msec
  refractory_per = refractory_per*1000;
case '.1msec'
  total_time = duration*10000; % timestamps
...
if refractory_per == 0
  ...
  while l > 0
    ...
    l = length(idx); % number to whipe out
    timestamps(idx) = [];
    % fprintf('%d,',l)
  end		
end	
