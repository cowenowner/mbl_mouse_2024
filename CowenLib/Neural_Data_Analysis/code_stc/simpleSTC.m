...
% 2. Compute both the spike-triggered and raw stimulus means and covariances
    ...
    if Msz < CriticalSize  % Check if stimulus is small enough to do in one chunk
        ...
        % Cowen: Use the version below if you want to include the STA in the STC
        % (it will be the first principal component in the STC)
        %STC = SS(iisp,:)'*(SS(iisp,:).*repmat(spvec,1,rowlen))/(nsp-1);
