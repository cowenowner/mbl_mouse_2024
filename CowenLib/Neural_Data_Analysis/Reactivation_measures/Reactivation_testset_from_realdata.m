function OUT = Reactivation_testset_from_realdata(RCT, nReps)
% function OUT = Reactivation_testset_from_realdata(RCT, nReps)
% 
...
%
% Assumptions: Since a snippet from behavior involving all neurons is taken
% and then inserted into sleep, the assumption is that all active cells
% during the behavioral event are a part of the reactivation event. Seems
% reasonable, but I could imagine situations where some subsets are not.
%
...
if nargin < 1 || isempty(RCT.RCT)
    ...
    RCT.data_set_type = 'from_real_data'; % alternative: 'construct_artificial_set'
