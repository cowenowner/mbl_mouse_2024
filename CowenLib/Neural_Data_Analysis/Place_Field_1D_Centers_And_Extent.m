function [O, V] = Place_Field_1D_Centers_And_Extent(x_dim, V, estimated_centers,filter_it,fld_loc_thresh_xdim, perc_peak)
% given a vector of bin values and estimated centers, determine the true
% location of the peak, center of mass, and the edges (drop to 80* of
% peak).
%
% fld_loc_thresh_xdim = the acceptable distance between the estimated
% center and the computed COM or peak. If the calculated distance exceeds
% this, then this estimate will be ignored.
%
% ASSUMES THE ESTIMATED CENTERS ARE SORTED!!!
%
% The user may need to smooth the data before it is passed in.
%
%
% Cowen 2011
...
for iCtr = 1:length(estimated_centers)
    ...
    O.Field_IX = false(size(x_dim));
    O.Field_IX(lowix:highix) = true;
