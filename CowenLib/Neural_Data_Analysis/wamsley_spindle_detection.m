function [detection,spindle_times_sec,spindle_recs] = wamsley_spindle_detection(C3_N2,C3,fs,target_fq)
...
%
% Cowen: Had to add a ceil() command to the window creation to convert to
% integers incase the fs was not divisible by 10.
% Added target_fq so that users could determine what the center frequency
% is.
if nargin < 4
    target_fq = 13.5;
end
signalmean = threshold_wamsley(C3_N2,fs,target_fq);
detection = wamsley(C3,fs,signalmean,target_fq);
...
spindle_times_sec = spindle_recs/fs;
    ...
    function signalmean = threshold_wamsley(C3nrem2,fs,fb)
...
%         fb = 13.5;
    ...
    function detection = wamsley(C3,fs,signalmean,fb)
...
%         fb = 13.5;
        ...
        for i=1:((length(current_data))-(fs*0.3));  % for the length of the signal, if the sum of 30 concurrent points = Fs*0.3, mark a spindle
            if sum(over(i:(i+(ceil(fs*0.3)-1))))==ceil(fs*0.3);
                locs_03(i,1)=1;
            end
        end
