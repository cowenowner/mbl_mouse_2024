function [ISIs_cleaned, INFO] = Spike_train_of_given_entropy_using_optimization(start_ISIs, target_entropy)
%
% INPUT:
%
% OUTPUT:
...
% ENTROPY: e = -sum(p.*log2(eps+p)); WILL NEED TO BIN

fun = @(x)(target_entropy - LocalVariance(x))^2; % THIS WORKED WHEN I MAKE LOCALVARIANCE() DO ABS(ISIs). I was trying to trick the objective function to convert to absolute value, but this fucked things up. Do it in LocalVariance.
fun = @(x)(target_entropy - LocalVariance(x))^2 + sum(x<0.005); % ??? Does not seem to break things but the mean() does not seem to do its job.
% STUFF THAT DID NOT WORK WELL>>.
% fun = @(x)abs(target_entropy - LocalVariance(x)) + abs(sum(x)-start_train_duration); % this seems to fail.
% fun = @(x)abs(target_entropy - LocalVariance(x) + .4*double(any(x<=0))); % Punish negative ISIs.
% fun = @(x)abs(target_entropy - LocalVariance(abs(x)) + abs((sum(abs(x))-duration_before_fmin)/duration_before_fmin)); % Punish negative ISIs.
% sum(x<=0) is to punish negative ISIs. Fails
%  fun = @(x)abs(target_entropy - LocalVariance(x)) ;
% fun = @(x)abs(target_entropy - LocalVariance(abs(x))) ;
% fun = @(x)(target_entropy - LocalVariance(abs(x)))^2 ; % maybe squaring converges faster?
