...
% [x] = ginput();
spindle_times = [4891649678.38487 4903675776.05835 ;5489087276.15992 5505444652.75137];
...
%% Find spindles in entire dataset.
pow_sp = abs(hilbert(filtfilt(filts{1},LFP.LFP)));
pow_ctrl = abs(hilbert(filtfilt(filts{2},LFP.LFP)));
threshold = 5000;
hvs_sig = conv_filter(pow_sp-pow_ctrl,hanning(LFP.sFreq));
se = find_intervals([LFP.t_uS,hvs_sig],threshold);

figure
plot(LFP.t_uS/60e6, LFP.LFP);
yyaxis right
plot(LFP.t_uS/60e6, hvs_sig);
