...
% fq_bands = {'theta' 'beta' 'gamma_50' 'gamma_80' 'HFO'};
fq_bands = {'gamma_50' 'gamma_80'};
...
if any(E.EventID == 'KetInjectionStart')
    ...
    intervals_around_drug_min{cnt} = [-20 -5; 2 30; 60 80];
...
if any(E.EventID == 'LDOPAInjectionStart')
    ...
    intervals_around_drug_min{cnt} = [-20 -5; 35 55; 65 85];
...
for iTE = 1:length(te)
    ...
    for iFreq = 1:length(fq_bands)
        for ii = 1:Rows(TIMES.(te{iTE}).IntervalUsec)
            ...
            % for plotting filtered trace
            %time=LFP.t_uS(ix1:ix2);
            %figure; plot(time(1000:1500),L(1000:1500))
            
            % do something crazy. Look at the response relative to power
            % in the frequency rather than the phase.

            
            % check if there are adjacent bands for the gamma_80 to get
            % focal 80
%             Lp_adj = {};
%             s_adj = {};
            if exist ('adj_bands','var')
                
                F_adj = SPEC_create_filters(adj_bands,LFP.sFreq);
                for b = 1:length(adj_bands)
                    L_adj{:,b} = filtfilt(F_adj{b},LFP.LFP(ix1:ix2));
                end
                L_adj_concat = cell2mat(L_adj);
                for i = 1:length(L_adj)
                    s_adj{:,i} = envelope_cowen(abs(L_adj_concat(:,i)));
                    % hilbert power
                    ss = abs(hilbert(L_adj_concat(:,i)));
                    ss = ss - mean(ss);
                    Lp_adj{:,i} = ss;
                end 
                % adj bands
                s_adj_concat = cell2mat(s_adj);
                Lp_adj_concat = cell2mat(Lp_adj);
                % gamma 80 power
                ss = abs(hilbert(L));
                ss = ss - mean(ss);
                Lp = ss;
                % power with envelope for intervals to get spikes
                s = envelope_cowen(abs(L));
                % checking for regression

                if regress_80
                    [coef_80_hilbert,bint_hilbert,Lp] = regress(Lp,[ones(Rows(Lp_adj_concat),1) Lp_adj_concat]);
                    [coef_80_env,bint_env,s] = regress(s,[ones(Rows(s_adj_concat),1) s_adj_concat]);
                else
                    % subtracting the average of the adj bands from 80
                    m_Lp_adj = mean(Lp_adj_concat')';
                    Lp = Lp - m_Lp_adj;
                    m_s_adj = mean(s_adj_concat')';
                    s = s - m_s_adj ;
                end
            else 
                ss = abs(hilbert(L));
