function [AC,x] = AutoCorrArray_Abhi(TS,bin_size,window_duration,intervals)
...
for iInterval = 1:Rows(intervals)
    ...
    for iC = 1:length(TStmp)
        if ~isempty(TStmp{iC}) && length(TStmp{iC}) > 2
            [AC(iC,:,iInterval),x] = Auto_corr(TStmp{iC},bin_size*10,nbins);
        else
            AC(iC,:,iInterval) = 0;
        end
    end
