function [first_in_burst, durations, n_per_burst, burst_id] = Burst_detector_Abhi(T, interval_thresh, min_spikes_per_burst, second_interval_thresh)
% INPUT: 
...
elseif isa(T,'cell')
    for ii = 1:length(T)
        if ~isempty(T{ii})
            if nargout == 0
                figure
                Burst_detector_Abhi(T{ii},interval_thresh, min_spikes_per_burst);
            else
                [first_in_burst{ii}, durations{ii}, n_per_burst{ii}, burst_id{ii}] = Burst_detector_Abhi(T{ii},interval_thresh, min_spikes_per_burst);
            end
        else
