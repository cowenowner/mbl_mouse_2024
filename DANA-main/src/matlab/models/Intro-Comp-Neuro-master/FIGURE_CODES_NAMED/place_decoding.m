...
%     xfit = fit(xvals', xhist, 'gauss1'); % BROKEN - Cowen found that fit
%     does not work
%     yfit = fit(yvals', yhist, 'gauss1');

% [xy] = fmgaussfit(xvals',yvals',xyhist); % for kicks I started to try this, but this 2d fit has 7 parameters - I was expecting 4 for a 2d gaussian- but maybe there is also rotation and something else? Not sure. Upshot- needs more work here.
% xhist2 = movmedian(xhist,5); % Funny - this I thought would help - but
% max works better.
[~,ix] = max(xhist); % fit based on the max - reasonable but noisy. -- actually - I am impressed.
    xfit.b1 = xvals(ix);
% yhist2 = movmedian(yhist,5);
[~,ix] = max(yhist);
    yfit.b1 = yvals(ix);

%     xfit.c1 = 40; % 40 works much better than 10.
%     yfit.c1 = 40;
    % try to make an informed estimate of the std. Assume 68% of data is in
    % +/- one std. Use prctiel to figure this out.
    xfit.c1 = sum(xhist>prctile(xhist(:),68))*(xvals(2)-xvals(1));
    yfit.c1 = sum(yhist>prctile(yhist(:),68))*(yvals(2)-yvals(1));
