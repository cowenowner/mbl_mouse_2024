...
% [B,C] = AutoCorr(t_usec{iWV}/100,5,40);
% figure;bar(C,B)
% figure
% hist(log(diff(t_usec{iWV}/1e3)),1000)
% xlabel('log ms')
...
%         lfp(BADIX) = nan;
        ...
        for iEvt = 2:2 %length(ET)
            ...
            if PMTM
                ...
                save([num2str(lfp_w)],'ans');
                PETH_W = ans;
               % save (['PETH_W_' (iLFP),PETH_W]);
                
                for ii =1:length(PETH_W.eeg_samples)
                    INTAN_Spectral_Analysis
                end
            end
