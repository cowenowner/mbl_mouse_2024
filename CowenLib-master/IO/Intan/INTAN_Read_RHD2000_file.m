function [OUT]=INTAN_Read_RHD2000_file(fname)
% function [OUT]=INTAN_Read_RHD2000_file(fname)
%
% Reads Intan Technologies RHD2000 data file generated by evaluation board
% GUI. Spits out the collected data and events. 
%
% INPUT: name of file base
% OUTPUT: OUT structure of the data.
% 
% NOTE: OUT.t_amplifier are in record numbers, not time.
%
% Adapted from "read_Intan_RHD2000_file Version 1.3, 10 December 2013"
%
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Cowen 2018. This function was just pretending to be a function and for some reason 
% loaded output into the workspace so I turned it into an actual function and removed a lot of the annoying fprintf to screen.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
OUT.spike_triggers = struct(spike_trigger_struct);
...
OUT.supply_voltage_channels = struct(channel_struct);
OUT.board_adc_channels = struct(channel_struct);
OUT.board_dig_in_channels = struct(channel_struct);
OUT.board_dig_out_channels = struct(channel_struct);
    ...
    if (signal_group_num_channels > 0 && signal_group_enabled > 0)
        ...
        for signal_channel = 1:signal_group_num_channels
            ...
            if (channel_enabled)
                switch (signal_type)
                    case 0
                        OUT.amplifier_channels(amplifier_index) = new_channel;
                        OUT.spike_triggers(amplifier_index) = new_trigger_channel;
                        amplifier_index = amplifier_index + 1;
                    case 1
                        OUT.aux_input_channels(aux_input_index) = new_channel;
                        aux_input_index = aux_input_index + 1;
                    case 2
                        OUT.supply_voltage_channels(supply_voltage_index) = new_channel;
                        supply_voltage_index = supply_voltage_index + 1;
                    case 3
                        OUT.board_adc_channels(board_adc_index) = new_channel;
                        board_adc_index = board_adc_index + 1;
                    case 4
                        OUT.board_dig_in_channels(board_dig_in_index) = new_channel;
                        board_dig_in_index = board_dig_in_index + 1;
                    case 5
                        OUT.board_dig_out_channels(board_dig_out_index) = new_channel;
...
%     fprintf(1, 'Allocating memory for data...\n');
    ...
    OUT.t_amplifier = zeros(1, num_amplifier_samples);
    ...
    OUT.board_dig_out_data = zeros(num_board_dig_out_channels, num_board_dig_out_samples);
...
%     fprintf(1, 'Reading data from file...\n');
    ...
    for i=1:num_data_blocks
        ...
        || (data_file_main_version_number > 1))
            OUT.t_amplifier(amplifier_index:(amplifier_index+59)) = fread(fid, 60, 'int32');
        else
            OUT.t_amplifier(amplifier_index:(amplifier_index+59)) = fread(fid, 60, 'uint32');
        end
...
%     fprintf(1, 'Parsing data...\n');
    ...
    for i=1:num_board_dig_in_channels
       mask = 2^(OUT.board_dig_in_channels(i).native_order) * ones(size(board_dig_in_raw));
       OUT.board_dig_in_data(i, :) = (bitand(board_dig_in_raw, mask) > 0);
    end
    for i=1:num_board_dig_out_channels
       mask = 2^(OUT.board_dig_out_channels(i).native_order) * ones(size(board_dig_out_raw));
...
%     supply_voltage_data = 74.8e-6 * supply_voltage_data; % units = volts
    ...
    num_gaps = sum(diff(OUT.t_amplifier) ~= 1);
    if (num_gaps == 0)
...
%     OUT.t_amplifier = OUT.t_amplifier / sample_rate;
%     t_aux_input = OUT.t_amplifier(1:4:end);
%     t_supply_voltage = OUT.t_amplifier(1:60:end);
%     t_board_adc =OUT.t_amplifier;
%     t_dig = OUT.t_amplifier;
%     t_temp_sensor = t_supply_voltage;
    ...
    if (notch_filter_frequency > 0)
        ...
        for i=1:num_amplifier_channels
            % COWEN: The INTAN notch filter aint so great. Future: Use
            % Notch_filter_cowen instead.
