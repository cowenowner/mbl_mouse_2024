function [wIH,wHO,SSE_vector,iter] = Neuralnet_bp_cube_force_out(wIH,wHO,in_pat,...
...
for iter=1:max_iter
   ...
   for sample = 1:nsamples
      ...
      % Compute the error
      % --------------------------------------------------------------------
      % find the maximum value for each row and make that the error for everyone.
      [i,j] = find(aO == max(max(aO)));
      E = zeros(size(aO));      
      E(i(1),:) = out_pat{sample}(i(1),:) - aO(i(1),:);       % Training error
      % --------------------------------------------------------------------
      % Backpropagage the error
      % --------------------------------------------------------------------
      % delta for ouput layer -- non-linear outputs
      % Just cube the outputs
