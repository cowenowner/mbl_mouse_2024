function [wIH,wHO,SSE_vector,iter] = Neuralnet_bp_sum_sqr_out(wIH,wHO,in_pat,...
...
for iter=1:max_iter
   ...
   for sample = 1:nsamples
      ...
      niO = [aH,bH] * wHO; 
      % Compute the activation
      aO = sum(pmntanh(niO).^3); 
      % --------------------------------------------------------------------
      % Compute the error
      % --------------------------------------------------------------------
      E            = sum(out_pat{sample}) - aO;       % Training error
      ...
      d2 = 3*sum(pmntanh(niO)).*(1-aO.*aO).*E; % 1x3
      d2 = repmat(d2,size(niO,1),1);
