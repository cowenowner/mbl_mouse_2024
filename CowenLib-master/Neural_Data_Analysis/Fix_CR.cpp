...
*    start timestamp
*    end timestamp
*    type of search (by timestamp, by record)
* 
* output:
*    [t, cr, SampFreq]
					  ...
					  int nINP, const mxArray *pINP[])
{
	...
	if (nINP != 2)
		mexErrMsgTxt("Call with fn_in and fn_out as inputs.");
	
	/* read inputs */
	fnlen = (mxGetM(pINP[0]) * mxGetN(pINP[0])) + 1;
	fnlen_out = (mxGetM(pINP[1]) * mxGetN(pINP[1])) + 1;
	...
	fn_out = (char *) mxCalloc(fnlen_out, sizeof(char)); 
	...
	while (fp <= postHeaderPos)
	{	
		fread(&tmp, sizeof(tmp),1,fp);
		fwrite(&tmp, sizeof(tmp),1,fp_out);
	}
