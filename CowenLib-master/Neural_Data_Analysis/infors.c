...
/*                  *************************************
  ...
  srand ( seed); // Cowen: modified this so that it is compatible with PCs (no drand48 in microsoft std libs)
   Read_Input();
...
/*      mxFree(responses); */
   ...
   while (s_throw > 0 && num_s > 0)
   {
     // s_start = drand48() * num_s;
      s_start = rand() * num_s;  // Cowen: modified this so that it is compatible with PCs (no drand48 in microsoft std libs)
