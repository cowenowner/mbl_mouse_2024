function [f] =Check_cluster_from_file(ts_dir, tt_dir, varargin)
...
t = Load_times('..')
% Load all of the spikes.
a_tsl = LoadTsObjects(tsl,'.');

for ii = 1:length(a_tsl)
    tet_id = get(a_tsl(ii),'tetrode_ID');
...
current_TT = [];
for current_t_file = 1:length(a_tsl)
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    spike_times = a_tsl(current_t_file);

    tmp_TT = [tt_dir filesep 'TT' num2str(get(a_tsl(current_t_file),'tetrode_ID')) ];
    % Don't reload the TT file if you don't have to.
    if strcmp(tmp_TT,current_TT)
    else
        current_TT = tmp_TT;
        [t, wv] = LoadTT0_nt(current_TT, Data(spike_times),1);
    end

    current_tfile_name = get(a_tsl(current_t_file),'object_ID');
    ...
    if ~isempty(t)
        ...
        [junk, tfile_name] = fileparts(current_tfile_name);
        ...
        drawnow
        if save_figures
            saveas(f(figure_count-1),[tfile_name fig_name '_ClustView.' fig_string ],fig_string);
        ...
        if draw_position
            ...
            if save_figures
                saveas(f(figure_count-1),[tfile_name fig_name '_PF.' fig_string],fig_string);
                close(f(figure_count-1))
            end
