...
T_rec = readNPY(fullfile(data_dir,'spike_times.npy')); %NOTE: there are times in here where spike times are identical between samples. I guess this can happen if spikes are simultaneous across channels maybe.
T_usec = (double(T_rec)/sFreq)*1e6; % convert to uSec
...
% T_usec(WV_recID+1) This would be the timestamp of each of the subset
% spikes
...
for iG = 1:length(groups_to_load)
    ...
    for ii = 1:length(gix)
        ...
        % reciDs.
    %         WV_ix = intersect(find(SPK_IX),1:length(WV_recID)); % need -1 as recID starts at 0
        ...
        figure; plot(single(squeeze(mean(WV(WV_ix,:,:),1,'omitnan'))))
        SP(cnt).dir = data_dir;
        SP(cnt).Cell_type = [];
        SP(cnt).Quality = [];
        SP(cnt).Notes = [];
