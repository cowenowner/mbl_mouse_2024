...
function  fig_cntx = VectCircPlot2Az_1way(typeplot, fig_cnt, ...
                 Vect_Az, VectorMean, ...
...
% Copyright C 2004, 2009  Thomas A. Jones.  All rights reserved.
...
%     Compass plot showing individual vector means
%     Rose diagram for a sample
%     Rose diagram for entire dataset
...
% Input variables:
%   typeplot: 0=rose diagram   1=compass plot on vector means 
...
%            columns in Azims
%   CatN: sample size for each of the q samples and total sample
...
% Output variable:
%   fig_cntx: Returned count on figures (figure number of last figure)
...
radius = 0.7;
radiusx = radius*1.1;  radiusxx = radius*0.8;  sra = radius*0.5;
...
% COMPASS PLOT OF CATEGORY VECTOR MEANS    
   ...
   CatX = zeros(Nd, 1);     CatY = zeros(Nd, 1);
   ...
   for iii = 1:Nd
      ...
      jjj = jjj + 5;
         if a > 1.571 & a < 4.712
          ...
          CatX(iii) = sin(a - dela)*radiusxx;
          CatY(iii) = cos(a - dela)*radiusxx;
         else
          acntX(iii) = sin(a + dela)*sra;
          acntY(iii) = cos(a + dela)*sra;         
          CatX(iii) = sin(a + dela)*radiusxx;
   ...
   fig_cnt = fig_cnt + 1;
   figure(fig_cnt)
   set(fig_cnt, 'Color', [0.95, 0.95, 0.95]);
   ...
   for iii = 1:Nd
       azlbl = num2str(CatN(iii));
       text(acntX(iii), acntY(iii), azlbl, 'Color', [0 0 1]);
       CatLbl = mat2str(IndepName(CatName(iii), :));
       text(CatX(iii), CatY(iii), CatLbl,  'Color', [0 0 1]);
   end
   axis off
   
   clear vectsX vectsY acntX acntY CatX CatY
